...
std::vector<Level> InitLevels() {
  ...
  lvl2.platforms = {
      {{SCREEN_WIDTH - 250, (SCREEN_HEIGHT - 150), 100, 100}, flower},
      {{0, 250, 200, 50}, basic},
      {{300, 350, 200, 50}, basic},
      {{600, 450, 200, 50}, basic},
      {{600, (float)(SCREEN_HEIGHT - 50), 10, 100},
       invisible}, // special fake platform for mob
      {{50, -200, 375, 50}, basic},
  ...
  lvl2.enemies = {{{lvl2.platforms[fake_platform_idx].position.x,
                    lvl2.platforms[fake_platform_idx].position.y},
                   {100, 100},
                   true,
                   fake_platform_idx,
                   roach}};
  ...
  lvl3.platforms = {
      {{0, 400, 450, 50}, basic},
      {{600, 600, 100, 100}, flower},
      {{800, 450, 200, 50}, basic},
      {{1000, 300, 200, 50}, basic},
      {{0, (float)(SCREEN_HEIGHT - 70), (float)SCREEN_WIDTH, 50}, invisible},
  };
  lvl3.enemies = {
      {{lvl3.platforms[2].position.x + lvl3.platforms[2].position.width / 2,
        lvl3.platforms[2].position.y},
       {100, 100},
       false,
       2}};
  lvl3.clouds = {{{100, -50, 150, 40}, 75.0f, 200, 800, true},
                 {{SCREEN_WIDTH, -50, 150, 40}, 75.0f, -100, 900, false}};
...
void UpdateGameplay(GameState &game, float delta) {
  ...
  if (playerToggle && game.isPlayerBurning) {
    ...
    if (game.burnTimer % 15 == 0) {
      ...
      Vector2 firePos = {game.player.position.x,
  ...
  // 1. Update Player
  updatePlayer(game.player, currentLvlData.platforms, currentLvlData.enemies,
...
void DrawGameplay(GameState &game) {
  ...
  // Transparent green fill DrawRectangleLinesEx(currentLvlData.exitZone, 2.0f,
  // GREEN);   // Solid outline DrawText("EXIT", (int)currentLvlData.exitZone.x
  // + 5, (int)currentLvlData.exitZone.y - 20, 20, GREEN); Draw Exit Zone
  if (game.texExit.id != 0) {
    Rectangle sourceRec = {0.0f, 0.0f, (float)game.texExit.width,
                           (float)game.texExit.height};
    ...
    Rectangle drawRect = {
        currentLvlData.exitZone.x -
            (drawWidth - currentLvlData.exitZone.width) / 2 + xOffset,
        currentLvlData.exitZone.y -
            (drawHeight - currentLvlData.exitZone.height) + yOffset,
  ...
  Vector2 playerPosVisual = {game.player.position.x - game.player.size.x / 2,
                             game.player.position.y - game.player.size.y};
  // DrawRectangleV(playerPosVisual, game.player.size, BLUE);
  // DrawRectangleLinesEx((Rectangle){playerPosVisual.x, playerPosVisual.y,
  // game.player.size.x, game.player.size.y}, 2.0f, BLACK);
  // --- HP BAR ---
  float barWidth = 6.0f;
  float barHeight = 50.0f;
  float barX = playerPosVisual.x + game.player.size.x +
               6.0f; // Position to the right of player
  float barY =
      playerPosVisual.y + (game.player.size.y - barHeight) /
                              2.0f; // Center vertically relative to player
  ...
  if (game.player.healthPoints <= 2)
    barColor = RED;
  else if (game.player.healthPoints <= 4)
    barColor = ORANGE;
  ...
  if (isEnemyActive()) {
    for (Enemy &enemy : currentLvlData.enemies) {
      ...
      Texture2D currentTex =
          (enemy.type == spider) ? game.texSpider : game.texRoach;
...
int main() {
  ...
  game.texintroScreen = LoadTexture("startScreen.png");
  ...
  if (game.texExit.id == 0) {
    std::cout
        << "ERROR: Could not load 'exit.png'. Is it in the resources folder?"
        << std::endl;
  }
  ...
  while (!WindowShouldClose() && !game.exitGame) {
    ...
    case TITLE:
      ...
      DrawTexturePro(game.texintroScreen,
                     (Rectangle){0, 0, (float)game.texintroScreen.width,
                                 (float)game.texintroScreen.height},
    ...
    case GAMEPLAY:
      ...
      break;
    case PAUSE:
      // Draw gameplay behind pause menu (simulated by re-drawing specialized
      // simple bg or just overlay) Ideally, capture gameplay to a texture, but
    ...
    case GAME_OVER: {
      ...
      DrawTexturePro(game.texGameOver,
                     (Rectangle){0, 0, (float)game.texGameOver.width,
                                 (float)game.texGameOver.height},
                     (Rectangle){0, 0, SCREEN_WIDTH, SCREEN_HEIGHT},
                     (Vector2){0, 0}, 0.0f, WHITE);

      DrawTexturePro(game.texGameOver2,
                     (Rectangle){0, 0, (float)game.texGameOver2.width,
                                 (float)game.texGameOver2.height},
                     (Rectangle){SCREEN_WIDTH / 2 - 150,
                                 SCREEN_HEIGHT / 3 + 150, 400, 200},
