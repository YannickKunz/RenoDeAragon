...
std::vector<Level> InitLevels() {
  ...
  // LEVEL 1
  Level lvl1;
  lvl1.backgrounds = std::make_tuple(LoadTexture("level1_daybackground.jpg"),
                                     LoadTexture("level1_nightbackground.jpg"));
  ...
  lvl1.enemy = {lvl1.platforms[0].position.x +
                    lvl1.platforms[0].position.width / 2,
                lvl1.platforms[0].position.y,
                {30, 30},
                false,
                0};
  lvl1.clouds = {{{100, 250, 200, 40}, 150.0f, 50, 600, true},
  ...
  lvl2.platforms = {
      ...
      {{0, (float)(SCREEN_HEIGHT - 50), (float)SCREEN_WIDTH, 50}, basic}};
  lvl2.enemy = {
      {lvl2.platforms[1].position.x + lvl2.platforms[1].position.width / 2,
       lvl2.platforms[1].position.y},
      {30, 30},
      false,
      1};
  lvl2.clouds = {{{200, 150, 180, 40}, 120.0f, 100, 500, true},
  ...
  lvl3.enemy = {
      ...
      false,
      2};
  lvl3.clouds = {{{300, 200, 150, 40}, 200.0f, 200, 900, true},
...
void UpdateGameplay(GameState &game, float delta) {
  ...
  // 1. Update Player
  updatePlayer(game.player, currentLvlData.platforms.data(),
  ...
  //    updateEnemy(currentLvlData.enemy, currentLvlData.platforms[pi].position,
  //    delta);
  //}
...
void DrawGameplay(GameState &game) {
  ...
  // Note: Enemy drawing is likely inside the enemy struct logic or missing from
  // original code explicitly, adding basic rect: Assuming enemy was drawn
  // previously or not crucial for refactor demo, skipping explicit draw call if
  // not found in original snippet logic. Draw Enemy
  if (!currentLvlData.platforms.empty()) {
    int pi = currentLvlData.enemy.patrolPlatformIndex;
    // We need 'delta' here. Since DrawGameplay doesn't typically take delta,
    // strictly speaking we should pass it, or just use GetFrameTime() directly
    // here.
    updateEnemy(currentLvlData.enemy, currentLvlData.platforms[pi].position,
                GetFrameTime());
  }
  ...
  if (game.isPlayerBurning) {
    Vector2 center = {game.player.position.x,
