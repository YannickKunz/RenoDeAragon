...
struct Level {
  ...
  Vector2 sunPosition;
  std::tuple<Texture2D, Texture2D> backgrounds; // First: Day, Second: Night
};

// void updatePlayer(Player &player, Rectangle *platformsLevel1, int
// platformsLevel1Length, Enemy &enemy, const float delta, Vector2 spawnPoint) {
...
// 				  plat.y <= (player.position.y + player.speed *
// delta)) { 			  hitObstacle = true;
// player.speed = 0.0f; 			  player.position.y = plat.y;
// break;
// 		  }
  ...
  while (!WindowShouldClose() && !exitGame) {
    ...
    case GAMEPLAY: {
      ...
      std::string debugText =
          ...
          "\nBG ID: " + std::to_string(bg.id) +
          " W: " + std::to_string(bg.width) +
          " H: " + std::to_string(bg.height);
      ...
      // Use BLEND_ADDITIVE for a "glowing" look, or BLEND_ALPHA for standard
      // transparency BeginBlendMode(BLEND_ADDITIVE);
      DrawTextureRec(
          lightLayer.texture,
          (Rectangle){0, 0, (float)screenWidth, (float)-screenHeight},
          (Vector2){0, 0}, Fade(WHITE, 0.2f)); // Apply opacity here!
                                               // EndBlendMode();
